...
app_state = {
    ...
    "LOG_HAFTASI": None,
    "GRUP_BAGLANTILARI": {},
    "USER_SESSIONS": {}  # hatirlatici vb. gecici veriler
}
...
def get_gspread_client():
    if os.path.exists(SERVICE_ACCOUNT_FILE):
        ...
        raise FileNotFoundError("Google Service Account anahtarı (service_account.json) bulunamadı!")
...
def grupEmojisiBul(grupAdi: str) -> str:
    ...
    if "TİGER" in temiz or "TIGER" in temiz:
        return "🐅"
    if "GENEL TOPLAM" in temiz:
        return "🏆"
    if "MASRAF" in temiz or "GİDER" in temiz:
        return "📉"
    if "KARGO" in temiz:
        return "📦"
    return "🔹"
...
def sistemeLogYaz(islemAdi: str, detay: str):
    try:
        ...
        now = datetime.datetime.now()
        gecerliHafta = f"{now.year}-{now.isocalendar()[1]}"
        
        if app_state["LOG_HAFTASI"] != gecerliHafta:
            rows = logSayfasi.get_all_values()
            if len(rows) > 1:
                logSayfasi.batch_clear([f"A2:C{len(rows)}"])
            app_state["LOG_HAFTASI"] = gecerliHafta
            
        tarihSaat = now.strftime("%d.%m.%Y %H:%M")
...
def menuKlavyesiOlustur(isGroup: bool):
    ...
    try:
        sh = get_spreadsheet()
        aktif_ad = bugununTarihiniAl()
        sayfa = sh.worksheet(aktif_ad)
    ...
    keyboard.append([InlineKeyboardButton("⏰ Hatırlatıcı Kur", callback_data="menu_hatirlatici")])
    keyboard.append([InlineKeyboardButton("🛠️ Komut Rehberi", callback_data="rehber")])
    
    return InlineKeyboardMarkup(keyboard)

# --- KOMUT METİNLERİ VE MOTORLAR ---
def rehber_metni():
    return (
        ...
        "<code>/kasasil TİGER 500</code> : Kasadan iptal/silme yapar.\n"
        ...
        "<code>/masrafsil 500 Yemek</code> : Son masrafı siler.\n"
        "<code>/gerial</code> : Sistemdeki en son işlemi geri alır.\n\n"
        "📊 <b>GÜNLÜK DÖNGÜ VE RAPORLAR</b>\n"
        "<code>/yenigun</code> : 🌅 (Kalkanlı) Yeni gün sayfasını açar, devirleri formülleri koruyarak aktarır.\n"
        ...
        "<code>/adminekle [ID]</code> : Yeni yetkili ataması yapar.\n"
...
def hucreyeVeriYaz_impl(komut: str, sutun_idx: int, isim: str, carp: int):
    p = komut.split(" ")
    if len(p) < 3:
        raise ValueError("Eksik komut. Örnek: /kasa TİGER 1500")
    ...
    for i, row in enumerate(tum_veriler[1:], start=2):
        if len(row) >= 2 and trKarakterCoz(row[1]) == grup:
            ...
            app_state["SON_ISLEM"] = {
                "sayfa": aktif_ad, "satir": i, "sutun": sutun_idx,
...
def masrafVerisiYaz_impl(komut: str, isim: str, carp: int):
    p = komut.split(" ")
    if len(p) < 3:
        raise ValueError("Eksik komut. Örnek: /masrafekle 500 Yemek")
    tutar = float(p[-1].replace(",", "."))
    ...
    for i, row in enumerate(tum_veriler[1:], start=2):
        ...
        if trKarakterCoz(col_i) == masraf:
            ...
            app_state["SON_ISLEM"] = {"sayfa": aktif_ad, "satir": i, "sutun": 10, "eskiDeger": mevcut, "grupAdi": masraf, "islemTuru": isim}
    ...
    if not masraf_guncellendi and carp == 1:
        # Boş satır bul
        ...
        app_state["SON_ISLEM"] = {"sayfa": aktif_ad, "satir": bos_satir, "sutun": 10, "eskiDeger": 0, "grupAdi": masraf, "islemTuru": "Yeni Masraf Ekleme"}
...
async def generic_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if app_state["SISTEM_KILIDI"] == "AKTIF" and user_id != KURUCU_ID:
        if any(text_lower.startswith(k) for k in ["/kasa", "/odeme", "/devir", "/masraf", "/gerial", "/hedef"]):
            await msg.reply_text("🛑 <b>SİSTEM KİLİTLİ!</b>\nKurucu acil durum kilidini açtı.", parse_mode=ParseMode.HTML)
            return

    try:
        if text_lower == "/rehber" or text_lower.startswith("/rehber@") or text_lower == "/komutlar":
        ...
        elif text_lower == "/kilit":
            if user_id == KURUCU_ID:
                app_state["SISTEM_KILIDI"] = "AKTIF"
                await msg.reply_text("🔒 <b>Sistem veri girişine kilitlendi.</b>", parse_mode=ParseMode.HTML)
        elif text_lower == "/kilitac":
            if user_id == KURUCU_ID:
                app_state["SISTEM_KILIDI"] = "PASIF"
                await msg.reply_text("🔓 <b>Sistem kilidi kaldırıldı.</b>", parse_mode=ParseMode.HTML)
            ...
            try:
                ...
                not_sayfasi = sh.add_worksheet(title="NOTLAR", rows=500, cols=3)
        ...
        elif text_lower.startswith("/yenigun"):
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🗑️ Masrafları SIFIRLA ve Yeni Gün Aç", callback_data="yenigun_onay_sil")],
                [InlineKeyboardButton("📋 Masrafları KORU ve Yeni Gün Aç", callback_data="yenigun_onay_tut")],
                [InlineKeyboardButton("❌ İPTAL ET", callback_data="yenigun_iptal")]
            ])
            await msg.reply_text(f"🌅 <b>Yeni Gün Açılışı ({bugununTarihiniAl()})</b>\n\nMasraflar temizlensin mi?", reply_markup=kb, parse_mode=ParseMode.HTML)
        elif text_lower.startswith("/yedekle"):
            if user_id != KURUCU_ID:
                await msg.reply_text("⛔ Sadece Kurucu yedek alabilir.", parse_mode=ParseMode.HTML)
                return
            await msg.reply_text("🛡️ <i>Yedek hazırlanıyor...</i>", parse_mode=ParseMode.HTML)
            url = f"https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID}/export?format=xlsx"
            gc = get_gspread_client()
            headers = {"Authorization": f"Bearer {gc.auth.token}"}
            res = requests.get(url, headers=headers)
            if res.status_code == 200:
                doc_bytes = io.BytesIO(res.content)
                doc_bytes.name = f"Sirket_Yedek_{datetime.datetime.now().strftime('%d_%m_%Y_%H%M')}.xlsx"
                await msg.reply_document(document=doc_bytes, caption="🛡️ <b>KARA KUTU YEDEĞİ ALINDI</b>", parse_mode=ParseMode.HTML)
            else:
                await msg.reply_text("❌ Yedekleme indirilemedi.", parse_mode=ParseMode.HTML)
...
async def get_dashboard_api():
    try:
        ...
        hedef = float(app_state.get("CIRO_HEDEFI") or 0)
        yuzde = min(100.0, round((toplamCiro / hedef) * 100, 1)) if hedef > 0 else 0.0
        
        return {
            "tarih": bugununTarihiniAl(),
            "devir": toplamDevir, "kasa": toplamKasa, "odenen": toplamOdenen,
            "kalan": toplamKalan, "ciro": toplamCiro, "hedef": hedef, "yuzde": yuzde,
            "gruplar": gruplar
        }
    except Exception as e:
        return {"error": str(e)}
...
<head>
  ...
  <style>
    body { background-color: var(--tg-theme-bg-color, #121212); color: var(--tg-theme-text-color, #ffffff); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 15px; margin: 0; }
    h2 { font-size: 18px; font-weight: 700; border-bottom: 1px solid #333; padding-bottom: 10px; display: flex; justify-content: space-between; align-items: center;}
    .date-badge { font-size: 11px; background: #333; padding: 4px 10px; border-radius: 20px;}
    .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 20px; }
    .card { background-color: var(--tg-theme-secondary-bg-color, #1e1e1e); border-radius: 12px; padding: 12px; }
    .stat-title { font-size: 10px; color: #888; text-transform: uppercase; margin-bottom: 5px; }
    .stat-value { font-size: 18px; font-weight: 800; }
    .green { color: #4ade80; } .blue { color: #60a5fa; } .yellow { color: #facc15; } .purple { color: #c084fc; } .red { color: #f87171; } .gray { color: #9ca3af; }
    .progress-container { margin-top: 20px; }
    .progress-bg { background: #333; border-radius: 10px; height: 10px; width: 100%; margin-top: 8px; overflow: hidden; }
    .progress-bar { background: linear-gradient(90deg, #3b82f6, #8b5cf6); height: 100%; border-radius: 10px; width: 0%; transition: width 1s; }
    .group-row { padding: 12px 0; border-bottom: 1px solid #2d2d2d; }
    .g-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
    .g-details { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; font-size: 11px; }
    .detail-item { background: #262626; padding: 6px; border-radius: 6px; text-align: center; }
    .detail-full { grid-column: span 2; }
  </style>
</head>
<body>
  <h2>Canlı CFO Paneli <span class="date-badge" id="d-tarih">--.--.----</span></h2>
  <div class="grid">
    <div class="card"><div class="stat-title">🔄 Önceki Devir</div><div class="stat-value gray" id="d-devir">0 ₺</div></div>
    <div class="card"><div class="stat-title">💰 Güncel Kasa</div><div class="stat-value blue" id="d-kasa">0 ₺</div></div>
    <div class="card"><div class="stat-title">💸 Ödenen</div><div class="stat-value red" id="d-odenen">0 ₺</div></div>
    <div class="card"><div class="stat-title">✂️ Üretilen Ciro</div><div class="stat-value purple" id="d-ciro">0 ₺</div></div>
    <div class="card"><div class="stat-title">🏦 Net Kalan</div><div class="stat-value green" id="d-kalan">0 ₺</div></div>
    <div class="card"><div class="stat-title">🎯 Ciro Hedefi</div><div class="stat-value yellow" id="d-hedef">0 ₺</div></div>
  </div>
  <div class="card progress-container">
    <div style="display:flex; justify-content:space-between; font-size:11px; font-weight:bold;"><span>Hedef İlerlemesi</span><span id="d-yuzde">%0</span></div>
    <div class="progress-bg"><div class="progress-bar" id="d-bar"></div></div>
  </div>
  <div style="font-size:15px; font-weight:600; margin: 20px 0 10px 0;">🏢 Aktif Ekipler ve Performans</div>
  <div class="card" id="d-list"></div>
  <script>
    if (window.Telegram && window.Telegram.WebApp) { Telegram.WebApp.expand(); Telegram.WebApp.ready(); }
    function fmt(n) { return Math.abs(n).toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + ' ₺'; }
    fetch('/api/dashboard').then(r => r.json()).then(data => {
      if (data.error) return;
      document.getElementById('d-tarih').innerText = data.tarih;
      document.getElementById('d-devir').innerText = fmt(data.devir);
      document.getElementById('d-kasa').innerText = fmt(data.kasa);
      document.getElementById('d-odenen').innerText = fmt(data.odenen);
      document.getElementById('d-ciro').innerText = fmt(data.ciro);
      document.getElementById('d-kalan').innerText = fmt(data.kalan);
      document.getElementById('d-hedef').innerText = data.hedef > 0 ? fmt(data.hedef) : 'Yok';
      document.getElementById('d-yuzde').innerText = '%' + data.yuzde;
      document.getElementById('d-bar').style.width = data.yuzde + '%';
      var html = '';
      data.gruplar.forEach(g => {
        html += '<div class="group-row"><div class="g-header"><div>🔹 <b>' + g.ad + '</b></div><div class="green"><b>' + fmt(g.kalan) + '</b></div></div><div class="g-details"><div class="detail-item">💰 Kasa: ' + fmt(g.kasa) + '</div><div class="detail-item">💸 Ödenen: ' + fmt(g.odenen) + '</div><div class="detail-item detail-full">✂️ Ciro: ' + fmt(g.komisyon) + '</div></div></div>';
      });
      document.getElementById('d-list').innerHTML = html || '<div style="color:#888;text-align:center;">Aktif ekip yok.</div>';
    });
  </script>
</body>
</html>"""
...
async def main():
    ...
    tg_app.add_handler(CommandHandler("menu", start_command))
    tg_app.add_handler(MessageHandler(filters.ALL, generic_message_handler))
    
    await tg_app.initialize()
    await tg_app.start()
    await tg_app.updater.start_polling()
    
    logger.info("CFO Bot ve Canlı Dashboard başlatıldı...")
    
    config = uvicorn.Config(app=fastapi_app, host="0.0.0.0", port=8080, log_level="warning")
